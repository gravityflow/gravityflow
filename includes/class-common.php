...
class Gravity_Flow_Common {
	 ...
	 * @return array
	 */
	public static function get_users_as_choices() {
		static $choices;
		...
		if ( ! isset( $choices[ $key ] ) ) {
			$role_choices = self::get_roles_as_choices( true, true );
			...
			$assignee_fields_as_choices = self::get_assignee_fields_as_choices( $form );

			if ( ! empty( $assignee_fields_as_choices ) ) {
				$field_choices = $assignee_fields_as_choices;
			}

			$email_fields_as_choices = self::get_email_fields_as_choices( $form );
	 ...
	 * @return array
	 */
	public static function get_assignee_fields_as_choices( $form = null ) {
		if ( empty( $form ) ) {
			$form_id = absint( rgget( 'id' ) );
			$form    = GFAPI::get_form( $form_id );
		}
	 ...
	 * @return array
	 */
	public static function get_email_fields_as_choices( $form = null ) {
		if ( empty( $form ) ) {
			$form_id = absint( rgget( 'id' ) );
			$form    = GFAPI::get_form( $form_id );
		}
